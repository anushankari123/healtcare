      ...
      {analysisResult && (
        <div className="space-y-6">
          ...
          <div className="flex space-x-3">
            <button
              onClick={() => {
                if (analysisForm.patientId && analysisResult) {
                  // Save this analysis to patient's records
                  saveAnalysisToPatient(analysisForm.patientId, analysisResult);
                }
              }}
              disabled={!analysisForm.patientId}
              className="flex-1 bg-blue-500 text-white py-3 px-4 rounded-lg hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center"
            >
