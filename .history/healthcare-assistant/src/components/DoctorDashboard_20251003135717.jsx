  ...
  if (data) {
    ...
    if (patient) {
      setSelectedPatient({
        ...
        gender: "Female",
        medical_records: data?.recent_records || [],
        predictions: []
      });
    }
  }
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className={`lg:col-span-1 ${
              ...
              {loading.patients ? (
                ...
                // In the Patients Tab section, update the patient list:
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2">
          {patient.statistics?.medical_records_count > 0 && (
            <span className="bg-blue-500 text-white text-xs px-2 py-1 rounded-full">
              {patient.statistics.medical_records_count} records
            </span>
          )}
    ...
    <div className="space-y-6">
        ...
        <div className="flex justify-between items-start">
          <div>
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              {selectedPatient.profile?.age && (
                <p><strong>Age:</strong> {selectedPatient.profile.age}</p>
              )}
              {selectedPatient.profile?.blood_type && (
                <p><strong>Blood Type:</strong> {selectedPatient.profile.blood_type}</p>
              )}
              {selectedPatient.profile?.height && selectedPatient.profile?.weight && (
                <>
                  <p><strong>Height:</strong> {selectedPatient.profile.height} cm</p>
                  <p><strong>Weight:</strong> {selectedPatient.profile.weight} kg</p>
                </>
              )}
              {selectedPatient.profile?.bmi && (
                <p><strong>BMI:</strong> {selectedPatient.profile.bmi}</p>
              )}
            </div>
          </div>
          <div className="text-right">
            ...
            {selectedPatient.last_login && (
              <p className={`text-xs ${darkMode ? 'text-purple-300' : 'text-purple-600'}`}>
                Last login: {new Date(selectedPatient.last_login).toLocaleDateString()}
              </p>
            )}
            {selectedPatient.statistics && (
              <p className={`text-xs ${darkMode ? 'text-purple-300' : 'text-purple-600'}`}>
                {selectedPatient.statistics.total_records || 0} records, {selectedPatient.statistics.total_predictions || 0} predictions
              </p>
            )}
          </div>
        </div>
      ...
      <div>
        <h3 className="text-xl font-bold mb-4 flex items-center">
          ...
          Medical Records ({selectedPatient.recent_records?.length || 0})
        </h3>
        ...
        {!selectedPatient.recent_records || selectedPatient.recent_records.length === 0 ? (
          ...
          <div className="space-y-4">
            {selectedPatient.recent_records.map((record, index) => (
              <div key={record.id} className={`p-4 rounded-xl border ${
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <div className={`text-sm p-2 rounded ${
                      ...
                      <p>Severity: {record.severity || 'Not specified'}</p>
                    </div>
                  </div>
                </div>

                {/* Additional medical information */}
                {(record.previous_conditions || record.current_medications || record.allergies) && (
                  <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
                    {record.previous_conditions && (
                      <div>
                        <h6 className="font-semibold mb-1 text-purple-600">Previous Conditions</h6>
                    ...
                    {record.current_medications && (
                      <div>
                        <h6 className="font-semibold mb-1 text-blue-600">Current Medications</h6>
                        <p className={`p-2 rounded ${
                          darkMode ? 'bg-slate-600' : 'bg-white'
                        }`}>
                          {record.current_medications}
                    ...
                    {record.allergies && (
                      <div>
                        <h6 className="font-semibold mb-1 text-red-600">Allergies</h6>
                        <p className={`p-2 rounded ${
                          darkMode ? 'bg-slate-600' : 'bg-white'
                        }`}>
                          {record.allergies}
                ...
                {record.doctor_notes && (
                  <div className="mt-3">
                    <h6 className="font-semibold mb-1 flex items-center text-green-600">
                      <Stethoscope className="h-3 w-3 mr-1" />
                      Doctor Notes
                    </h6>
                    <p className={`text-sm p-2 rounded ${
                      darkMode ? 'bg-slate-600' : 'bg-white border border-green-200'
                    }`}>
                      {record.doctor_notes}
                    </p>
                  </div>
          ...
          <div className="space-y-4">
            {selectedPatient.predictions.map((prediction, index) => (
              <div key={prediction.id} className={`p-4 rounded-xl border ${
                ...
                <div className="flex justify-between items-start mb-3">
                  <div>
                    <h4 className="font-bold text-lg">{prediction.predicted_condition || prediction.predicted_disease}</h4>
                    <p className="text-sm">
                      Confidence: {Math.round((prediction.confidence_score || prediction.confidence) * 100)}%
                    </p>
                    {prediction.predicted_severity && (
                      <p className="text-sm">
                        Predicted Severity: <span className="capitalize">{prediction.predicted_severity}</span>
                      </p>
                    )}
                    {prediction.doctor_comments && (
                      <p className="text-sm mt-1 italic">
                        <strong>Doctor comments:</strong> {prediction.doctor_comments}
                      </p>
                    )}
                  </div>
                  <div className="flex items-center space-x-2">
                ...
                {prediction.symptoms && prediction.symptoms.length > 0 && (
                  <div className="mb-3">
                    <h5 className="font-semibold mb-1 text-sm">Analyzed Symptoms:</h5>
                ...
                {prediction.recommendations && (
                  <div>
                    <h5 className="font-semibold mb-1 text-sm flex items-center text-purple-600">
                      <Activity className="h-3 w-3 mr-1" />
                      Recommendations
                    </h5>
                    <p className={`text-sm p-2 rounded ${
                      darkMode ? 'bg-slate-600' : 'bg-white border border-purple-200'
                    }`}>
                      {prediction.recommendations}
                    </p>
                  </div>
                ...
                {/* Doctor Analysis Info */}
                {prediction.analyzed_by_doctor && (
                  <div className="mt-2 text-xs text-gray-500">
                    Analyzed by: Dr. {prediction.analyzed_by_doctor?.user?.first_name} {prediction.analyzed_by_doctor?.user?.last_name}
                  </div>
                )}
              </div>
            ))}
      ...
      {selectedPatient.profile?.emergency_contact && (
        <div className={`p-4 rounded-xl border-2 ${
          darkMode ? 'border-red-600 bg-red-900/20' : 'border-red-400 bg-red-50'
        }`}>
          <h3 className="text-lg font-bold mb-2 flex items-center text-red-600">
            <AlertCircle className="h-5 w-5 mr-2" />
            Emergency Contact
          </h3>
          <p className="text-sm">{selectedPatient.profile.emergency_contact}</p>
        </div>
      )}
    </div>
  )}
