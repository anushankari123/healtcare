...
{activeTab === 'quickActions' && (
  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
    ...
    <RecentActivitySection darkMode={darkMode} />
        ...
        {/* Patients Tab */}
        // Patients Tab - Updated to fetch real data
{activeTab === 'patients' && (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  {patient.statistics?.predictions_count > 0 && (
                    <span className="bg-green-500 text-white text-xs px-2 py-1 rounded-full">
                      {patient.statistics.predictions_count} analyses
                    </span>
                  )}
                ...
                {selectedPatient.predictions.map((prediction) => (
                  <div key={prediction.id} className={`p-4 rounded-xl border ${
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                      <div>
                        <p><strong>Status:</strong> {prediction.status || 'Completed'}</p>
                        {prediction.related_medical_record && (
                          <p><strong>Related Record:</strong> #{prediction.related_medical_record}</p>
                        )}
                      </div>
                      <div>
                        {prediction.alternative_diagnoses && prediction.alternative_diagnoses.length > 0 && (
                          <p><strong>Alternative diagnoses considered</strong></p>
                        )}
                      </div>
                    </div>
                    ...
                    {prediction.doctor_notes && (
                      <div className="mt-3 p-3 rounded bg-blue-50 border border-blue-200">
                        <h5 className="font-semibold text-sm mb-1">Doctor's Notes:</h5>
                        <p className="text-sm">{prediction.doctor_notes}</p>
                      </div>
                    )}
                  </div>
                ))}
