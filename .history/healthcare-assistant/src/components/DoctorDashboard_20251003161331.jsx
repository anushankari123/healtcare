...
// Updated fetchPatientDetails function
const fetchPatientDetails = async (patientId) => {
  setLoading(prev => ({ ...prev, patient: true }));
  
  try {
    // Fetch patient details - this endpoint works based on your logs
    const { data: patientData } = await apiCall(`/doctor/patients/${patientId}/`);
    
    if (patientData) {
      // Since the medical-records and predictions endpoints return 404,
      // we'll use whatever data comes from the main patient endpoint
      setSelectedPatient({
        ...patientData,
        // Use the data that comes from the API, fallback to empty arrays
        medical_records: patientData.medical_records || [],
          ...
          {(realPatients.length > 0 ? realPatients : patients).map((patient) => (
            ...
            >
              ...
              <div className="text-xs mt-2 opacity-70">
                ID: {patient.id} | Has records: {patient.medical_records ? 'Yes' : 'No'}
              </div>
            </div>
          ))}
        </div>
    ...
    <div className={`lg:col-span-2 ${
      ...
      {loading.patient ? (
        ...
        <div className="space-y-6">
          {/* Debug header to see what data we have */}
          <div className={`p-4 rounded-lg ${darkMode ? 'bg-yellow-900/20 border-yellow-600' : 'bg-yellow-50 border-yellow-200'} border`}>
            <h4 className="font-bold text-yellow-800 dark:text-yellow-200">Debug Info:</h4>
            <p className="text-sm">Medical Records: {selectedPatient.medical_records ? selectedPatient.medical_records.length : 'None'}</p>
            <p className="text-sm">Predictions: {selectedPatient.predictions ? selectedPatient.predictions.length : 'None'}</p>
            <p className="text-sm">Data keys: {selectedPatient ? Object.keys(selectedPatient).join(', ') : 'No data'}</p>
          </div>
            ...
            <div className="flex justify-between items-start">
              <div>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <p><strong>DOB:</strong> {selectedPatient.date_of_birth || selectedPatient.dob || 'Not provided'}</p>
              ...
              <div className="text-right">
                ...
                <p className={`text-xs ${darkMode ? 'text-purple-300' : 'text-purple-600'}`}>
                  Joined: {new Date(selectedPatient.date_joined || selectedPatient.created_at).toLocaleDateString()}
                </p>
              </div>
            </div>
          </div>

          {/* Medical Records - Flexible display */}
          <div>
            ...
            {!selectedPatient.medical_records || selectedPatient.medical_records.length === 0 ? (
              <div className={`text-center py-8 ${
                ...
                <p className="text-sm mt-2">
                  {selectedPatient.medical_records ? 'No records in array' : 'medical_records field is null/undefined'}
                </p>
              </div>
            ) : (
              <div className="space-y-4">
                {selectedPatient.medical_records.map((record, index) => (
                  <div key={record.id || index} className={`p-4 rounded-xl border ${
                    darkMode ? 'border-slate-600 bg-slate-700' : 'border-gray-200 bg-gray-50'
                  }`}>
                    <div className="flex justify-between items-start mb-3">
                      <h4 className="font-bold">
                        Medical Record {record.id ? `#${record.id}` : `#${index + 1}`}
                      </h4>
          ...
          {/* AI Predictions History - Flexible display */}
          <div>
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-bold flex items-center">
            ...
            {!selectedPatient.predictions || selectedPatient.predictions.length === 0 ? (
              <div className={`text-center py-8 ${
                ...
                <p className="text-sm mt-2">
                  {selectedPatient.predictions ? 'No predictions in array' : 'predictions field is null/undefined'}
                </p>
              </div>
            ) : (
              <div className="space-y-4">
                {selectedPatient.predictions.map((prediction, index) => (
                  <div key={prediction.id || index} className={`p-4 rounded-xl border ${
                    ...
                    <div className="flex justify-between items-start mb-3">
                      <div>
                        <h4 className="font-bold text-lg">
                          {prediction.predicted_disease || prediction.disease_name || `Prediction ${index + 1}`}
                        </h4>
                        <p className="text-sm">
                      ...
                      <div className="flex items-center space-x-2">
                        {prediction.approved_by_doctor ? (
                          ...
                          <button
                            onClick={() => approveAnalysis(prediction.id || index)}
