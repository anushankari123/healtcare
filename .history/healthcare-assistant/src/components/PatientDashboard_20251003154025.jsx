...
const PatientDashboard = ({ user, token, onLogout }) => {
  ...
  if (data) {
    // Ensure we're not duplicating records by replacing the entire array
  ...
  try {
    ...
    if (recordData) {
      ...
      if (predictionData) {
        ...
        // Only fetch prediction history, NOT medical records again
        fetchPredictionHistory();
  ...
  return (
    ...
    darkMode
      ...
      <div className="flex-1 px-4 pb-8">
        ...
        {/* Profile Tab */}
        {activeTab === 'profile' && (
          <div className={`flex flex-col min-h-screen w-full ${darkMode ? 'bg-slate-900' : 'bg-gray-50'} p-6 lg:p-8`}>
            <div className={`flex-1 rounded-xl shadow-lg border p-6 ${darkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200'}`}>
  ...
  <div className="w-full min-h-[70vh] flex flex-col">
    <div className={`flex-1 flex flex-col h-full ${
      darkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200'
    } rounded-xl shadow-lg border p-0 w-full`}>

      <div className="p-6 border-b border-gray-200 dark:border-slate-700 w-full">
        <h2 className="text-2xl font-bold flex items-center">
      ...
      <div className="flex-1 flex flex-col overflow-hidden w-full">
        {loading.records ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center py-12">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">Your symptom analysis records will appear here</p>
          </div>
        ) : (
          <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6 w-full">
            {medicalRecords.map((record) => (
              ...
              >
                <div className="flex flex-col md:flex-row justify-between md:items-start mb-2 gap-4 w-full">
                  <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
                  <div>
                    ...
                    <div className={`text-sm p-3 rounded-lg ${darkMode ? 'bg-slate-600' : 'bg-white'}`}>
                      {record.symptoms}
                    </div>
                  </div>
                  <div>
        ...
        {/* History Tab */}
        {activeTab === 'history' && (
  <div className={`min-h-screen w-full ${darkMode ? 'bg-slate-900' : 'bg-gray-50'}`}>
    <div className="w-full max-w-none p-6 lg:p-8">
      <div className={`rounded-xl shadow-lg border ${darkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200'} w-full`}>
        <div className="p-6">
          <h2 className="text-2xl font-bold mb-6 flex items-center">
          ...
          {loading.history ? (
            ...
            <div className={`flex flex-col items-center justify-center min-h-[400px] w-full ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
            ...
            <div className="space-y-4 w-full">
              {predictionHistory.map((prediction, index) => (
                <div key={prediction.id} className={`p-6 rounded-xl border w-full ${
