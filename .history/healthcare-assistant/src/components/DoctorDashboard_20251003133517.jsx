...
useEffect(() => {
  ...
  const fetchPatientDetails = async (patientId) => {
    setLoading(prev => ({ ...prev, patient: true }));
    // Simulate API call and set mock patient data
    setTimeout(() => {
      const patient = patients.find(p => p.id === patientId);
      if (patient) {
        setSelectedPatient({
          ...
          medical_records: [
            {
              id: 1,
              symptoms: "Persistent headache, nausea, sensitivity to light",
              duration: "3 days",
              severity: 7,
  ...
  const runAIAnalysis = async () => {
    ...
    setTimeout(() => {
      setAnalysisResult({
        predictions: [
          { condition: "Common Cold", probability: 78 },
          { condition: "Seasonal Allergies", probability: 65 },
          { condition: "Sinusitis", probability: 45 }
        ],
        confidence: 0.78,
        matchedSymptoms: symptomsArray.slice(0, 3),
        inputSymptoms: symptomsArray,
        analysisDate: new Date().toLocaleString(),
        patientInfo: analysisForm.patientId ? patients.find(p => p.id == analysisForm.patientId) : null
      });
        ...
        {activeTab === 'quickActions' && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
            ...
            <div className={`${
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold">{patients.reduce((sum, p) => sum + p.pending_reviews, 0)}</p>
                </div>
            ...
            <div className={`${
              ...
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium opacity-90">AI Analyses</p>
            ...
            <div className={`lg:col-span-3 ${
              ...
              <div className="space-y-4">
                {[
                  { action: "New patient registered", patient: "Jane Doe", time: "2 hours ago", type: "new" },
                  { action: "AI analysis completed", patient: "Bob Johnson", time: "4 hours ago", type: "analysis" },
                  { action: "Medical record updated", patient: "Alice Smith", time: "6 hours ago", type: "update" }
                ].map((activity, index) => (
